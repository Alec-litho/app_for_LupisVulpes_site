...
export default async function setColors(colors,input, setFinish) {
    ...
    let isExists = await fetch('http://localhost/app_for_lupisvulpes-site/root/public/api/v1/art/if_exists',{headers:headers,method:'post',body:JSON.stringify(...colorsIds)}).then(res=>res.json())//check if art already exists
...
async function storeColors(colors, headers) { 
    ...
    let result = await Promise.all(colors.map(async(color, indx, arr) => {//What i want to do is call Promise.all on the array returned by map in order to convert it to a single Promise before awaiting it.
        ...
        let colorId = await fetch('http://localhost/app_for_lupisvulpes-site/root/public/api/v1/color', {headers: headers,method: 'post',body: JSON.stringify(colorObj)}).then(res=>res.json()).catch(err => console.log(err))//store obj and get its id
        console.log(colorId);
